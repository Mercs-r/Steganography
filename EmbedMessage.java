...
{
JButton open = new JButton("Load image"), embed = new JButton("Embed"),
save = new JButton("Save into new file"), reset = new JButton("Reset"),home = new JButton("Back to main page");
...
if(sourceImage == null){
JOptionPane.showMessageDialog(this, "NO IMAGE TO EMBED MESSAGE",
"ORIGINAL IMAGE NOT AVAILABLE", JOptionPane.ERROR_MESSAGE);
return;

}

else if("".equals(mess)){
JOptionPane.showMessageDialog(this, "NO MESSAGE TO EMBED",
"MESSAGE NOT AVAILABLE", JOptionPane.ERROR_MESSAGE);
return;
}
String pin = JOptionPane.showInputDialog("Enter 4 digit pin");
int foo = Integer.parseInt(pin);
embedInteger(sourceImage,foo,0, 0);
embeddedImage = sourceImage.getSubimage(0,0,sourceImage.getWidth(),sourceImage.getHeight());
embedMessage(sourceImage, mess);
JLabel l = new JLabel(new ImageIcon(embeddedImage));
embeddedPane.getViewport().add(l);
this.validate();
JOptionPane.showMessageDialog(this, "MESSAGE EMBEDDED SUCESSFULLY.YOUR PIN IS:" +pin,
...
embedInteger(img, messageLength, 32, 0);

byte b[] = mess.getBytes();
for(int i=0; i<b.length; i++)
embedByte(img, b[i], i*8+64, 0);
